...
**Resilience details worth knowing (you wrote these, so own them):**
- **Turn lock** prevents two concurrent requests from corrupting one conversation's history.
- **History sanitization** (`sanitizeHistoryWindow`) trims a truncated window so it never starts with an orphan `tool_result` or ends with an unanswered `tool_use` — both of which the Anthropic API rejects. This is the kind of detail that proves you actually ran this thing.
...
- **Be careful conflating the two AI systems.** If an interviewer asks "what does the agent use," the answer is **Claude/Anthropic**. The Groq/Llama work is the *separate WhatsApp concierge and vendor extractor*. Mixing these up is the easiest way to look like you don't know your own stack.
