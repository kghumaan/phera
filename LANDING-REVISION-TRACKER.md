...
- [ ] **#4 — Shuttle flight-collection + assignment (IN PROGRESS).** Most infra already exists; this connects it. Slices:
  ...
  3. [x] **Assignment logic — SHIPPED + TESTED.** `lib/transportation/assign-shuttles.ts` is a pure, deterministic algorithm: each guest → the earliest shuttle departing after they land (+ buffer) with room for their whole party; greedy by arrival, parties kept together, capacity respected. **14 vitest cases** (`tests/assign-shuttles.test.ts`) cover buffer, capacity, party-too-large, all-full, no-shuttle, no-info, ordering, determinism — all green. `lib/transportation/suggest-assignments.ts` wires real data → preview (`buildShuttleAssignmentPreview`) + draft writer (`applyShuttleAssignments` → `pending` reservations to confirm).
  4. **Edge cases → admin tracking** — the algorithm already classifies every unplaceable guest (`no_arrival_info` / `no_shuttle_after_arrival` / `all_full` / `party_too_large`). REMAINING: a dashboard panel (slice 3b/4) that runs the preview, shows assignments + a "needs attention" list, and applies the drafts.
