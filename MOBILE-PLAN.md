...
- **2026-07-06 (session 11)** — **Full web-parity audit (six batches, all pushed).** Four deep behavioral specs of the web app (admin data pages / schedule-rooms-tasks-messaging-concierge / guest portal / agent+auth+access APIs) drove a screen-by-screen reconciliation. (1) **Palette:** tag chips now hash into 8 rani/sky/plum/butter stops via shared `tag-color` (web `lib/utils/tag-color.ts` + mobile mirror, sync-tested), avatar fallbacks rebuilt from ramp mid-stops, web room side tints + task columns re-pointed to ramps. (2) **Guest list:** row tags + plus-one line, latest-RSVP status derivation, name/email/phone search, web add-guest defaults. **Overview:** wedding summary card, web stat formulas (head-count vs records, pending = maybe+blank), real 6-step setup roadmap with completion queries. **Responses:** web tabs' stats + alpha chips + plus-one/song/maybe rows. (3) **Planner:** full SSE union (tool_start/tool_done + Undo, error frames, questions_required inline form → /api/agent/answer, upgrade/upload/data-panel/faq/venue/broadcast/pairing cards), confirm card with Edit-note + alwaysAllow; fixed a React-batching bug that dropped text after tool events. (4) **Guest portal:** new Travel & Stay page (travel_sections + legacy fallback), flight form → travel-details (RSVP-gated), conditional details hub, home venue/CTA gating + welcome_text + 30.44-day countdown, full RSVP payload (plus-one/song/maybe/side/consent/comments). (5) **Rooms:** side-tinted occupant slots + tag pills; concierge prefers the service-role conversations API (bearer); task tags brand. (6) **Settings & Publish** (publish confirm, integrations editing, checklist) + **Collaborators** invites (wedding_invites + /api/invites/send) + invite acceptance on SIGNED_IN. Known deliberate deltas: no registry/where-to-shop/transportation guest pages, no event-detail carousels, Cultural Guide stays in the hub, RSVP skips the web Account Creation step (gate identifies guests). Web bug found (not fixed): web overview's room completion queries wedding_rooms by UUID but the table keys on slug. 1445 web + 11 mobile tests green.

- **2026-07-06 (session 10)** — **Zero-setup live backend, palette, PIN-entry parity, demo tour.** Production Supabase/API defaults baked into `mobile/src/lib/config.ts` (preview now explicit `EXPO_PUBLIC_PREVIEW=1`); exact brand palette (`PALETTE`: rani pink / sky blue / plum / butter yellow / black + warm-white opacity ramps) added to web + mobile tokens with the sync test covering it, `COLORS` re-pointed to palette stops, inline colors swept; top breathing room reverted (screens start at the safe area; guest-entry back button is an overlay so forms center); guest gate rebuilt as `PinEntryGate` matching web PinEntry (corner garlands, stacked logo, italic serif headline, password + find-your-name steps, honors `pin_entry_*` customisations); `wedding_password` moved to admin-only `wedding_secrets` (two-phase migration `20260705`/`20260705b` — phase 2 runs only after this branch deploys). **First-run personas are now fully covered:** couple → login, guest → invitation-link entry, and NEW curious-downloader → runtime demo tour (`inMockMode()` = compile-time preview OR runtime demo; "Just exploring? Browse a sample wedding →" on the welcome fork + login enters the fixture wedding as its couple; pink "Demo wedding — tap to exit" chip exits; session-only, works in live builds — also satisfies App Review's demo-access ask). Verified live: demo tour end-to-end, prod auth error round-trip, real-wedding gate; 1445 web + 9 mobile tests green.

- **2026-07-05 (session 9)** — **Live production verification + first-launch onboarding.** With phera.io/www/supabase allowlisted: real Supabase auth round-trip verified, guest portal renders a real production wedding (Aarav & Ananya) with its theme background, access API rejects wrong passwords — zero page errors. (Harness note: sandbox Chromium can't use the MITM proxy; scratchpad relay.mjs forwards localhost:9099→supabase / :9098→www.phera.io — test-only.) NEW onboarding: first launch lands on /welcome — 3-slide full-screen carousel ending in the couple/guest fork; guest path → /guest-entry parses invitation links and validates slugs against the live weddings table; flag persists. Expo Go reality: iOS Expo Go tops out at SDK 54 → use `npx expo run:ios` or an EAS dev build.
