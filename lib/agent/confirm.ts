...
export async function resolveAgentAction(args: ResolveActionArgs): Promise<void> {
  ...
  if (!approve) {
    ...
    try {
      ...
      const published = (result as { websitePublished?: { url?: string; slug?: string } } | null)?.websitePublished;
      if (published?.url && published.slug) onEvent({ type: 'website_published', url: published.url, weddingSlug: published.slug });
      const resolution = {
        status: 'confirmed',
