...
import { stampAuthUserNames } from '../couple-names';
...
export const weddingTools: AgentToolDefinition[] = [
  ...
  },
  {
    ...
    execute: async (input, ctx) => {
      ...
      if (updates.couple_name || updates.partner1_name || updates.partner2_name) {
        // Keep the auth user record findable in the Supabase user list
        // (matters most for anonymous pre-signup sessions, which otherwise
        // have no identifying fields at all). Owner-checked and best-effort.
        await stampAuthUserNames(ctx.supabase, ctx.weddingUuid, {
          couple_name: (updated.couple_name as string) ?? '',
          partner1_name: (updated.partner1_name as string) ?? '',
          partner2_name: (updated.partner2_name as string) ?? '',
        });
      }
      return {
