...
export const askTools: AgentToolDefinition[] = [
  {
    ...
    risk: 'read',
    description:
      'Use this to ask the user one or more specific questions and collect typed answers via proper input fields (shown one at a time). ALWAYS prefer calling this over asking for structured info in prose — names, dates, times, a choice from options, etc. Group related questions into one call. Types: "text" (short answer), "textarea" (long answer), "date" (single date picker), "date_range" (start→end range picker — use this for the wedding/celebration dates instead of two separate date questions; the answer is "YYYY-MM-DD to YYYY-MM-DD", or a single date for one-day events), "time" (time picker), "single_select" (pick one of options), "multi_select" (pick several of options). Provide "options" for the select types. Keep date/time prompts neutral — do not name specific events in them.',
    inputSchema: {
      type: 'object',
      properties: {
        questions: {
          ...
          items: {
            ...
            properties: {
              ...
              type: { type: 'string', enum: ['text', 'textarea', 'date', 'date_range', 'time', 'single_select', 'multi_select'] },
