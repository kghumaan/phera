...
export type AgentStreamEvent =
   ...
   *  so they can forward it to guests. `weddingSlug` is the current admin slug
   *  (unchanged, or freshly renamed from a placeholder) — the client compares
   *  it against its own route param and navigates if publish just renamed it,
   *  since every subsequent request keyed on the old slug would otherwise 404. */
  | { type: 'website_published'; url: string; weddingSlug: string }
