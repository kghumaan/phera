...
docs/DEV-ROADMAP.md documents the (paused) outreach-engine task sequence; consult it only when outreach work resumes.
...
mobile/              # React Native app (Expo + Tamagui) — own package.json,
                     # excluded from the Next.js build. See docs/MOBILE-PLAN.md.
```
...
| Document | Purpose |
|----------|---------|
| `docs/AGENT-PLAN.md` | The Phera Agent plan — active focus, see above. |
| `docs/MOBILE-PLAN.md` | React Native mobile app (Expo + Tamagui) grand plan + living screen tracker. Branch `claude/react-native-mobile-app-1s5eu5`. |
| `docs/DEV-ROADMAP.md` | 24-task step-by-step engineering roadmap for the (paused) outreach engine. |
...
| `docs/TODO-CONCIERGE-AUTH.md` | Open gaps in Concierge identity/consent verification. |
| `docs/TOKEN-STRATEGY.md` | Model-cost strategy per environment/tier. |
| `docs/WHATSAPP-GROUPS-SETUP.md` | WhatsApp group creation/broadcast setup via the couple's own number. |
| `docs/ARCHITECTURE_EXPLAINER.md` | Deeper architecture walkthrough. |
| `docs/DESIGN_GUIDELINES.md` | Design guideline notes (see also `lib/theme/tokens.ts`). |
| `docs/LAUNCH-PLAN-B2C.md` | B2C launch plan. |
