  ...
  if (useVerticalScroll) {
    return (
      ...
      >
        ...
        >
          {/* Mock header — mirrors AppHeader's Container pt + logo wrapper
              height exactly so the floating Menu pill in VerticalScrollLayout
              (which uses the same pt + height) lines up vertically with the
              logo at every breakpoint. */}
          <Box sx={{
            display: 'flex',
            alignItems: 'center',
            ...
            {/* Right-side mock cluster (WhatsApp / Going / KG avatar) is
                intentionally empty here — these controls now live inside the
                VerticalScrollLayout drawer (opened via the floating Menu pill
                in the top-right). The WhatsApp icon will be reintroduced
                later when the community/channel feature ships. */}
