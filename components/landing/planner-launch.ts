  ...
  if (!sessionPromise) {
    sessionPromise = (async () => {
      try {
        ...
        // Tag where the anonymous session came from (user_metadata.source), so
        // internal testing against the prod database is filterable in the
        // Supabase user list: local-dev = a localhost session, landing = a real
        // visitor. Survives signup conversion (same user id).
        const source =
