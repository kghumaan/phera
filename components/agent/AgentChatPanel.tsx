...
const ONBOARDING_OPENER =
  ...
  'here to make planning feel less hectic and a lot more fun.';

const ONBOARDING_OPENER2 =
  'Answer the questions on the right, or just brain-dump below with any details or notes you already have. ' +
...
export function AgentChatPanel({
  ...
  // permanent, tell the agent so it acknowledges and continues. Derived from
  // the shared AuthContext (which is already listener-driven via
  // onAuthStateChange) instead of a one-off getSession() call here — a raw
  // getSession() on mount used to occasionally read the stale pre-callback
  // anonymous session while Supabase was still finishing the OAuth token
  // exchange, flashing the signup card back on for a moment before
  // self-correcting. Gating on authLoading (in addition to loadingHistory)
  // means nothing anon-only renders until the real session is known.
  const { user: authUser, isLoading: authLoading } = useAuth();
  const [isAnonSession, setIsAnonSession] = useState(false);
  const signupReturnHandledRef = useRef(false);
  useEffect(() => {
    if (loadingHistory || authLoading) return;
    const anon = authUser?.is_anonymous === true;
    setIsAnonSession(anon);
    if (!anon && authUser && !signupReturnHandledRef.current) {
  ...
  }, [loadingHistory, authLoading, authUser, weddingSlug]);
  ...
  const handleEvent = useCallback((event: AgentStreamEvent) => {
    ...
    // Publishing can rename a placeholder draft slug to a names-based one —
    // every subsequent request in this panel is keyed off the `weddingSlug`
    // prop, so a stale route param would 404 the next turn. Swap the browser
    // straight to the new admin URL; React re-renders this panel with the
    // fresh prop from the route.
    if (event.type === 'website_published' && event.weddingSlug !== weddingSlug) {
      router.replace(`/admin/${event.weddingSlug}/assistant`);
    }
    setItems((prev) => {
      const next = [...prev];
      switch (event.type) {
  ...
  // The create-account nudge renders in the form pane, not as a chat bubble —
  // it's a real form like the rest, not a second thing competing with a
  // handoff button for attention in the same breath.
  const signupItem = items.find(
    (i): i is Extract<ChatItem, { kind: 'signup' }> => i.kind === 'signup' && i.status === 'pending'
  );
  const hasSignupCard = !!signupItem;
  const formPaneOpen =
    hasSignupCard || !!pendingQuestions || hasFaqReview || hasVenueCards || hasPairing || hasBroadcast || hasDataPanel || facts.length > 0;
  ...
  const hasActivePanel =
    hasSignupCard || !!pendingQuestions || hasFaqReview || hasVenueCards || hasPairing || hasBroadcast || hasDataPanel;
  ...
  return (
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: { xs: 0, md: minHeight } }}>
            ...
            {items.map((item, index) =>
              item.kind === 'confirm' ? (
                ...
                item.status === 'done' ? (
                  ...
                  // Rendered in the form pane while pending (see hasSignupCard) —
                  // nothing inline, so it never doubles up next to a handoff button.
                  null
                )
              ) : item.kind === 'questions' ? (
      ...
      <Box sx={{ p: { xs: 1.25, md: 2 } }}>
        ...
        {/* Canned questions a couple can tap instead of typing — most useful
            right when a decision menu (single/multi_select) is up. Always
            answered by a normal chat turn, so it's the full model, never the
        ...
        {awaitingQuestions && !busy && !signupGate &&
          ...
          // the composer path would treat the pill as that question's ANSWER —
